...
parser.add_argument("--save_modelq", action="store_true")
parser.add_argument("--save_modelq_path", type=str, help="path to save model")
...
if args.train_path:
    ...
    df['rationale_labels'] = df['rationale_labels'].apply(
        lambda s: torch.tensor(np.asarray([int(i) for i in s.split(',')]), dtype=torch.long))
else:
...
if args.do_test:
    if args.test_path:
        ...
        df_test['rationale_labels'] = df_test['rationale_labels'].apply(
            lambda s: torch.tensor(np.asarray([int(i) for i in s.split(',')]), dtype=torch.long))
    else:
...
if args.do_validation:
    if args.dev_path:
        ...
        df_val['rationale_labels'] = df_val['rationale_labels'].apply(
            lambda s: torch.tensor(np.asarray([int(i) for i in s.split(',')]), dtype=torch.long))
    else:
...
tokenizer_SP = tokenizer.batch_encode_plus(df.seeker_post, add_special_tokens=True, max_length=args.max_len,
                                           pad_to_max_length=True, return_attention_masks=True)
...
if args.do_validation:
    val_tokenizer_RP = tokenizer.batch_encode_plus(df_val.response_post, add_special_tokens=True,
                                                   max_length=args.max_len, pad_to_max_length=True,
    ...
    val_tokenizer_SP = tokenizer.batch_encode_plus(df_val.seeker_post, add_special_tokens=True, max_length=args.max_len,
                                                   pad_to_max_length=True, return_attention_masks=True)
...
if args.do_test:
    test_tokenizer_RP = tokenizer.batch_encode_plus(df_test.response_post, add_special_tokens=True,
                                                    max_length=args.max_len, pad_to_max_length=True,
                                                    return_attention_masks=True)
    ...
    test_tokenizer_SP = tokenizer.batch_encode_plus(df_test.seeker_post, add_special_tokens=True,
                                                    max_length=args.max_len, pad_to_max_length=True,
                                                    return_attention_masks=True)
...
if args.do_validation:
    val_dataset = TensorDataset(val_input_ids_SP, val_attention_masks_SP, val_input_ids_RP, val_attention_masks_RP,
                                val_labels, val_rationales, val_rationales_trimmed)
    validation_dataloader = DataLoader(val_dataset, sampler=SequentialSampler(val_dataset), batch_size=args.batch_size)

if args.do_test:
    test_dataset = TensorDataset(test_input_ids_SP, test_attention_masks_SP, test_input_ids_RP, test_attention_masks_RP,
                                 test_labels, test_rationales, test_rationales_trimmed)
...
for epoch_i in range(0, args.epochs):
    ...
    if args.do_validation:
        ...
        for batch in validation_dataloader:
            ...
            total_eval_accuracy_rationale += flat_accuracy_rationale(logits_rationale, label_rationale_ids,
                                                                     label_empathy_ids, rationale_lens, axis_=2)

            pos_f1_empathy, micro_f1_empathy, macro_f1_empathy = compute_f1(logits_empathy, label_empathy_ids, axis_=1)
            macro_f1_rationale = compute_f1_rationale(logits_rationale, label_rationale_ids, label_empathy_ids,
                                                      rationale_lens, axis_=2)
...
def print_size_of_model(model, label=""):
    ...
    print("model 1: ",label,' \t','Size (KB):', size/1e3)
    os.remove('temp.p')
    return size


def print_size_of_model2(model, label=""):

    model.load_state_dict(torch.load('/Users/neko/Documents/EPIOME/output/sample_IP.pth'))
    modelq = torch.quantization.quantize_dynamic(model, {nn.Module}, dtype=torch.qint8)
    torch.save(modelq.state_dict(), args.save_modelq_path)
    #print_size_of_model2(modelq,"int8")

    #torch.save(model.state_dict(), "temp.p")
    size=os.path.getsize(args.save_modelq_path)
    print("model compressed: ",label,' \t','Size (KB):', (size)/1e3 - 900)
    #os.remove('temp.p')
    return size
...
if args.save_model:
    ...
    print_size_of_model2(model,"int8")

    #torch.save(model.state_dict(), args.save_modelq_path)
    # print("Printed weights of the model is down below")
    # for k, v in model.named_parameters():
    #     print(k, v)
    
    
    # print("Total weights of the model is down below")
    # print(model)

    # model.load_state_dict(torch.load(args.save_model_path))
    # model.eval()
    # print("Test Eval")
    # print_size_of_model(model,"int32")
    ...
    # model.load_state_dict(torch.load('/Users/neko/Documents/EPIOME/output/sample_IP.pth'))
    # modelq = torch.quantization.quantize_dynamic(model, {nn.Module, nn.Linear(4,4)}, dtype=torch.qint8)
    # torch.save(modelq.state_dict(), args.save_modelq_path)
    # print_size_of_model2(modelq,"int8")
